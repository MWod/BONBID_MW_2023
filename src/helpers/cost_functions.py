...
def dice_focal_loss_monai_multichannel(prediction : tc.Tensor, target : tc.Tensor, **kwargs) -> tc.Tensor:
    """
    Averaged Dice and Focal losses based on MONAI implementation.
    """
    no_channels = prediction.size(1)
    for i in range(no_channels):
        if i == 0:
            loss = dice_focal_loss_monai(prediction[:, i:i+1, :, :, :], target[:, i:i+1, :, :, :], **kwargs)
        else:
            loss += dice_focal_loss_monai(prediction[:, i:i+1, :, :, :], target[:, i:i+1, :, :, :], **kwargs)
    loss = loss / no_channels
    return loss
